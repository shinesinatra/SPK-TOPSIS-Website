<?php
    ...
    class PDF extends FPDF {
        function Header() {
            // $this->Image('../gambar/topsis.png', 10, 5, 20);
    ...
    $pdf->Cell(0,10,'Data Kriteria SPK Siswa Berprestasi ' .date('Y'),0,1,'C');
    $pdf->Ln(5);
    // Tampilan tabel
    $pdf->SetFont('Times','B',12);
    $pdf->Cell(45, 7, 'KODE', 1, 0, 'C');
    $pdf->Cell(100, 7, 'NAMA', 1, 0, 'C');
    $pdf->Cell(45, 7, 'BOBOT', 1, 0, 'C');
    $pdf->Cell(10, 7, '', 0, 1);
    // Tampilan data
    $pdf->SetFont('Times','',12);
    $sql = mysqli_query($koneksi, "SELECT * FROM kriteria");
    while ($row = mysqli_fetch_array($sql)) {
        $pdf->Cell(45, 7, $row['kode'], 1, 0, 'C');
        $pdf->Cell(100, 7, $row['nama'], 1, 0, 'L');
        $pdf->Cell(45, 7, $row['bobot'], 1, 1, 'C');
    }
    $pdf->Ln(50);
    $pdf->SetFont('Times','',12);
    $pdf->SetX(135);
